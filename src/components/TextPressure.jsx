...
const TextPressure = ({
  ...
  const setSize = () => {
    ...
    // Account for max width expansion (150% = 1.5x) and italic slant
    let newFontSize = (containerW / (chars.length / 2)) / 1.6;
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach(span => {
          ...
          const wdth = width ? Math.floor(getAttr(d, 100, 150)) : 100;
          const wght = weight ? Math.floor(getAttr(d, 100, 900)) : 400;
          const slntVal = italic ? Math.floor(getAttr(d, 0, 14)) : 0;
          const alphaVal = alpha ? getAttr(d, 0, 1).toFixed(2) : 1;

          span.style.opacity = alphaVal;
          span.style.fontVariationSettings = `'wght' ${wght}, 'wdth' ${wdth}`;
          span.style.fontStyle = italic ? `oblique ${slntVal}deg` : 'normal';
        });
      }
